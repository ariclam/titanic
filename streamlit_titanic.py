...
test_feature = scaler.fit_transform(X_test)

model = LogisticRegression()
model.fit(train_feature,y_train)

train_score = model.score(train_feature,y_train)
...
if choice == 'Overview':
    ...
    max_parh  = max(data['Parch'])
    parch = st.slider("Parch",0,max_parh,1)
    ...
    prediction_pro = model.predict_proba(new_data)

    if prediction[0] == 1:
        st.subheader('Passenger {} would have survived with a probability of {}%'.format(name,round(prediction_pro[0][1]*100,2)))
    else:
        st.subheader('Passenger {} would have not survived with a probability of {}%'.format(name,round(prediction_pro[0][0]*100,2)))